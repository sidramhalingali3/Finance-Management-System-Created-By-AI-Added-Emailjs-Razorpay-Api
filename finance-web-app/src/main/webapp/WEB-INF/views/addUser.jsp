...
<body>
    <div class="container login-container" style="max-width: 500px;">
        <div class="header-actions" style="margin-bottom: 1.5rem; padding-bottom: 0.5rem;">
            ...
            <a href="users" class="btn btn-outline" style="padding: 0.5rem 1rem; font-size: 0.875rem;">&larr; Back</a>
        </div>

        <% 
            String userError = request.getParameter("userError");
            if (userError != null && !userError.isEmpty()) {
                if ("empty_fields".equals(userError)) {
        %>
                    <div class='alert alert-error'>Please fill all fields.</div>
        <%      } else if ("insert_failed".equals(userError)) { %>
                    <div class='alert alert-error'>Failed to create user. Username might already exist.</div>
        <%      } else { %>
